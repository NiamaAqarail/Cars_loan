...
public class ClientServiceImpl implements ClientService {
    ...
    @Override
    public void addClient(Integer userId, Client client) {
        ...
        if (clientRepository.existsById(userId)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Client already exists for user id " + userId);
        }

        client.setUser(user);
        client.setId(user.getId()); // Ensure ID consistency
    ...
    public void deleteClient(Integer id) {
        ...
        // Remove association with User before deleting
        clientRepository.findById(id).ifPresent(client -> {
            client.setUser(null);
            clientRepository.flush(); // Ensure change is committed before deletion
        });
